...
The Netty API is very "fine-grained". It's not an application framework. But many frameworks, such as Finagle, Ratpack and Play are built on it. Maybe there's room for a JRuby application framework built on Netty. Or maybe we can bridge Netty to Rack.
